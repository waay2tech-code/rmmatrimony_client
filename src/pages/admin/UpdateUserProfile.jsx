...
const UpdateUserProfile = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        setIsLoading(true);
        const response = await userService.admingetProfile(id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      setIsLoading(true);
      await userService.adminupdateProfile(formData,id);
  ...
  const addPhoto = async () => {
    ...
    try {
      ...
      const response = await userService.adminuploadPhoto(formData,id);
  ...
  const deletePhoto = async (index) => {
    ...
    try {
      ...
      await userService.admindeletePhoto(photoObj.url,id);
    ...
    <>
      ...
      <div className="mb-6 text-center">
        <img 
          ...
          alt="Profile Preview" 
          className="mx-auto h-32 w-32 rounded-full object-cover mb-2 border-2 border-red-500"
          onError={(e) => {
